...
```{r echo =FALSE, dpi = 600, fig.width=12}
...
```{r echo=FALSE, warning=FALSE, dpi =600, fig.width=12}
...
```{r echo =FALSE, warnings = FALSE, message = FALSE, dpi = 600, fig.width=12}
...
Como decíamos, se generaron durante la semana pasada `r nrow(AEETMED_tweets)` tuits con la etiqueta #AEETMED, con `r length(unique(AEETMED_tweets$screenName))` usarios distintos tuiteando en algún momento de la semana. Estos fueron los 5 tuits más retuiteados:
...
```{r echo = FALSE, dpi = 600, fig.width=10}
# Todos los tweets
...
```{r, echo=FALSE, include =FALSE, dpi=600, fig.width=10}
...
```{r,  echo=FALSE, eval = FALSE, include =FALSE, dpi=600, fig.width=10}
## Most retweeted
...
```{r word-cloud, echo=FALSE, message=FALSE, dpi=600}
library(wordcloud)
